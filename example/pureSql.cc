...
int main(int argc, char* argv[]) {
 ...
 ON `Role`.`id`=`u_Role`.`role_id`  ORDER BY `u`.`id`";//反冒号无法从控制台正确录入，但可以通过后台服务器
  ...
  add(1, "代码哥", 44);
  add(2, "攻城师", 23);
  add(3, "程序员", 21); //使用方法： ./main "SELECT * from users_test"
  q.query(string("INSERT into users_test(id, name, age) values (?,?,?);"))(4, "编程王", 32);
  EXPECT_EQUAL(make_tuple("程序员", 21), (q("select name, age from users_test where id = 3;").template r__<string, int>()));
